...
.category-list
  v-row.pa-0.ma-0.px-4
      p.text-h6.font-weight-medium.pt-4.mb-0 Your Sales and Expenses in a nutshell
    ...
    vue-horizontal-list(
      ...
      template(v-slot:default="{item}")
        v-card(outlined).align-center.rounded-xl
            ...
            p.my-4.category-text.font-weight-medium.text-h6(:class="item.colorName + '--text'") {{"RM " + item.amount.toString()}}
  ...
  methods: {
    ...
    goToChallengeDetail (id) {
      if (id === 0) {
        ...
        this.$router.push('/expensesdetails1')
      }
    }
  }
}
